...
export var time_stay_in : float = 1.0
...
#### BUILT-IN ####
func _ready() -> void:
	var __ = distance_variable.add_variable("distance_variable_factor", distance_variable_factor, 2)
...
func enter_state() -> void:
	var __ = time_variable.add_variable("time", time_stay_in, 0)
...
func get_chance_value() -> float:
	...
	var difficulty_factor = (1-owner.difficulty) * difficulty_variable.get_value()
	return (max(0,time_factor+distance_factor+offensive_factor+difficulty_factor)*100)*global_variable.get_value()
