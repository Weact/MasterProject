...
func _input(event: InputEvent) -> void:
	if event is InputEventKey:
		if event.is_action_pressed("minimize"):
			...
			var map_valentin_player = get_tree().get_root().get_node_or_null("MapV/Player/Player")
			var map_lucas_player = get_tree().get_root().get_node_or_null("MapL/Player/Player")
